...
由于 Codex 本身的上下文限制，本技能只能分为两个阶段来运行，才能保证效果可用。第一阶段完成 PDF 拆分、页面识别、风格参考图生成和提示词生成；第二阶段新开一个 Codex 会话逐页生图并合成 PDF。

默认情况下，第一阶段结束后 Codex 会停下来等待用户手动回复 **“继续”**。用户可以在运行技能时的提示词中加入 **“自动进入第二阶段”**、**“自动继续”**、**“不用等我确认”**、**“第一阶段后直接继续”** 类似的话，则第一阶段完成后会直接进入第二阶段。
